...
export async function generateMetadata({ params: { slug } }) {
    try {
      const response = await fetch(
        `${API_END_POINT}/wp-json/wp/v2/pages?parent=384&slug=${slug}&_embed&acf_format=standard`, 
        { next: { revalidate: 20 } }
      );
      ...
      const currentUrl = `${process.env.NEXT_PUBLIC_SITE_URL || `${SITE_URL_POINT}/solutions/${slug}`}`;
...
const page = async ({ params: { slug } }) => {
  try {
    ...
    // 1. Fetch and translate Solutions page data
    const response = await fetch(
      `${API_END_POINT}/wp-json/wp/v2/pages?parent=384&slug=${slug}&_embed&acf_format=standard`,
      { next: { revalidate: 50 } }
    );
    const rawSolutionData = await response.json();
    const { texts: solutionTexts, paths: solutionPaths } = extractTextForTranslation(rawSolutionData);
    const translatedSolutionTexts = await translateText(solutionTexts, language);
    const translatedSolutionData = injectTranslatedText(rawSolutionData, solutionPaths, translatedSolutionTexts);
    ...
    return (
      <div className="max-w-[2500px] mx-auto bg-white">
        <Navbar />
        {translatedSolutionData.map((val) => (
          <div key={val.id}>
            <Solutions
              val={val}
              slug={slug}
              blogapi={translatedBlogData}
              rawBlogData={rawBlogData}
            />
          </div>
        ))}
        <Footer />
